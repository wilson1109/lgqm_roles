...
def main() -> int:
    ...
    for fragment in FORBIDDEN_SKILL_FRAGMENTS:
        if fragment in content:
            print(f"[FAIL] {skill_dir.name}: forbidden template/evidence fragment: {fragment}")
            return 1
    for generic_type in GENERIC_STEP1_TYPES:
        if f"**{generic_type}**" in content:
            print(f"[FAIL] {skill_dir.name}: generic Step 1 type: {generic_type}")
            return 1
    ...
    if len(step3) != 2 or step3[1].split("---", 1)[0].count("\n- ") < 4:
        print(f"[FAIL] {skill_dir.name}: Step 3 missing substantial character-specific bullets")
        return 1

    tmp = pathlib.Path(tempfile.mkdtemp(prefix=f"{skill_dir.name}-verify-"))
    os.symlink(skill_dir, tmp / skill_dir.name)
    return subprocess.run([sys.executable, str(repo_root / "verify_batch.py"), str(tmp)]).returncode
